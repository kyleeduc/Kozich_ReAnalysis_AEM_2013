...
```{r knitr_settings, eval=TRUE, echo=FALSE, cach=FALSE}
opts_chunk$set("tidy"=TRUE)
opts_chunk$set("echo"=FALSE)
opts_chunk$set("eval"=TRUE)
opts_chunk$set("warning"=FALSE)
opts_chunk$set("cach"=TRUE)
...
shared_file <- read.table(file=shared_file_name, header=TRUE, stringsAsFactors=FALSE)
...
for (d in unique_days) {
  subset <- shared_file[days == d, -c(1,2,3)]
  
  n_samples_per_day[as.character(d)] <- nrow(shared_file[days == d, ])
  total_seqs_per_day[as.character(d)] <- sum(subset)
  mean_seqs_per_day_per_sample[as.character(d)] <- mean(apply(subset, 1, sum), na.rm = TRUE)
}

data <- data.frame(
  Day = unique_days,
  `Seqs. per sample (total)` = mean_seqs_per_day_per_sample,
  `Total seqs` = total_seqs_per_day,
  `N samples` = n_samples_per_day
)
...
kable(
  ...
  row.names = FALSE,
  align = 'lcrc',
  col.names = c("Day", "Seqs. per<br>sample (total)", "Total<br>seqs", "Number of<br>samples"),
  escape = FALSE
)
```
